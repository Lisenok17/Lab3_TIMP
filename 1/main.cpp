...
SUITE(KeyTest) 
{
    ...
    TEST(LargeLetters) {
        modAlphaCipher test(L"СТУЛ");
        CHECK(true);
    }
    TEST(LargeAndSmallLetters) {
        modAlphaCipher test(L"СТУЛстол");
        CHECK(true);
    }
    TEST(EmptyKey) {
    ...
    TEST(Key_with_a_space) {
        CHECK_THROW(modAlphaCipher test(L"стул стол"), cipher_error);
    }
    TEST(Key_with_invalid_characters_number) {
        CHECK_THROW(modAlphaCipher test(L"стул6стол"), cipher_error);
    }
...
SUITE(DecryptTest) 
{
   TEST_FIXTURE(KeyAB_fixture, LargeLetters) {
    ...
    TEST_FIXTURE(KeyAB_fixture,TextWithNumber) {
        wstring cipher_text = L"стул123сто";
        CHECK_THROW(pointer->decrypt(cipher_text),cipher_error);
    }
    TEST_FIXTURE(KeyAB_fixture,TextWithSymbol) {
        wstring cipher_text = L"стул\стол";
        CHECK_THROW(pointer->decrypt(cipher_text),cipher_error);
    }
    TEST_FIXTURE(KeyAB_fixture,TextWithASpace ) {
        wstring cipher_text = L"стул стол";
